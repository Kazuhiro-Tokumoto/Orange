...
//! 共有できない (`oag-pow` の `randomx` モジュールを見よ)。
//!
//! そこで**採掘器を渡さない。作り方を渡す**。[`HasherFactory`] は
//! それぞれの作業スレッドの上で呼ばれ、そのスレッドだけが使う採掘器を
//! 建てる。採掘器そのものは生まれた場所から動かない。この crate に
//! **`unsafe` は要らない。**
//!
//! 作り方が何を抱えて渡るかは呼び出し側が決める。ノードは fast モードの
//! データセット (2 GB) を 1 本だけ建て、それを作り方に持たせて全スレッドで
//! 共有している (`oag_pow::randomx::SharedDataset`)。スレッドごとに増える
//! のは VM の 2 MB だけである。light モードはキャッシュを共有していない
//! ので、1 スレッドあたり 256 MB 要る。
//!
//! # 仕事の配り方
//!
